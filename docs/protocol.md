...
The client connects to `{endpoint}/tunnel` over `wss://` (the PHP package's
config provides the endpoint; the client normalizes `https://` to `wss://` and
appends `/tunnel` when no path is present). Optional query parameters:
...
`url` is the public URL that routes to this tunnel. The client derives its
subdomain from the `url`'s host for any subsequent reconnect in the same
session.
